...
<section class="mb-5">
    <div class="row">
        <a href="/datawisata" ><b class="text-primary">Kembali</b> /
            <input name="search" type="text" value="{{request('search')}}">
        </a>
    </div>


        <h1 class="mb-5 text-center font-extrabold leading-none md:text-5xl xl:text-6xl dark:text-dar">
        {{$category}}
        </h1>

        @foreach ($posts as $post)

            <h1 class="max-w-2xl text-4xl dark:text-dark">
                <a href="/posts/{{$post->slug}}">{{$post->title}}</a>
            </h1>
            <h5 class="mb-5">
                <a href="/petawisata">{{$post["address"]}}</a>
            </h5>
